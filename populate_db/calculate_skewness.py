...
def calculate_overall_skew(input_df):
    ...
    # current_period = input_df.loc[(input_df['Bill Date'] == current_date), 'period'].values[0]
    
    input_df['subperiod'] = input_df['Bill Date'].apply(lambda x: DatetoSubperiod(x))
    # current_subperiod = input_df.loc[(input_df['Bill Date'] == current_date), 'subperiod'].values
...
def calculate_territory_wise_skew(input_df, territory_df):
    ...
    for territory in territory_list:
        ...
        if sum(temp_val):
            ...
            avg_territory_sale_val[territory] = temp_val
        if sum(temp_volm):
            avg_territory_sale_volm[territory] = [round(100*x/sum(temp_volm),2) for x in temp_volm]
        else:
            avg_territory_sale_volm[territory] = temp_volm
...
def calculate_customer_wise_skew(input_df, customer_df):
    ...
    for customer in customer_list:
        ...
        if sum(temp_val):
            ...
            avg_customer_sale_val[customer] = temp_val
        if sum(temp_volm):
            avg_customer_sale_volm[customer] = [round(100*x/sum(temp_volm),2) for x in temp_volm]
        else:
            avg_customer_sale_volm[customer] = temp_volm
...
def calculate_sku_wise_skew(input_df, sku_df):
    input_df.sort_values(by='Bill Date',inplace=True)
    ...
    sku_details_df1 = input_df[['Item Name','Month Name', 'period', 'subperiod',\
                                'Quantity in KG/LTR', 'Amount To Customer']]
    sku_details_df1['Item Name'] = sku_details_df1['Item Name'].str.upper()

    test_df = sku_details_df1.groupby(['Item Name','Month Name','period', 'subperiod'],as_index=False)\
    ['Quantity in KG/LTR', 'Amount To Customer'].sum()

    test_df.rename(columns={'Item Name':'sku_name'}, inplace=True)
    sku_list = sku_df['sku_name'].drop_duplicates(keep = 'first').tolist()

    sku_sale_val, sku_sale_volm = {}, {}
    current_period_sale_val, current_period_sale_volm = {}, {}
    for sku in sku_list:
        subperiod_sale_val, subperiod_sale_volm = {}, {}
        for period in period_list:
            test_df1 = test_df[(test_df['period'] == period) & (test_df['sku_name'] == sku)]
            test_df1 = test_df1.groupby(['Month Name', 'period'],\
                                as_index=False)['Quantity in KG/LTR', 'Amount To Customer'].sum()
            temp_val, temp_volm = [], []
            for sub_period in subperiod_list:
        ...
        current_period_sale_val[sku] = subperiod_sale_val[current_period]
        current_period_sale_volm[sku] = subperiod_sale_volm[current_period]
        
        del subperiod_sale_val[current_period]  
        del subperiod_sale_volm[current_period]
        
        sku_sale_val[sku] = pd.DataFrame(subperiod_sale_val, index=subperiod_list).T
        sku_sale_volm[sku] = pd.DataFrame(subperiod_sale_volm, index=subperiod_list).T
        
    weights = [0.125, 0.125, 0.25, 0.5]
    avg_sku_sale_val, avg_sku_sale_volm = {}, {}
    for sku in sku_list:
        temp_val, temp_volm = [], []
        for sub_period in subperiod_list:
            temp_val.append(weighted_mean_rem_outlier(sku_sale_val[sku][sub_period].values,\
                                                      weights, remove = True))
            temp_volm.append(weighted_mean_rem_outlier(sku_sale_volm[sku][sub_period].values,\
                                                       weights, remove = True))
        
        if sum(temp_val):
            avg_sku_sale_val[sku] = [round(100*x/sum(temp_val),2) for x in temp_val]
        else:
            avg_sku_sale_val[sku] = temp_val
        if sum(temp_volm):
            avg_sku_sale_volm[sku] = [round(100*x/sum(temp_volm),2) for x in temp_volm]
        else:
            avg_sku_sale_volm[sku] = temp_volm
    
    return {
        'val' : avg_sku_sale_val,
        'volm' : avg_sku_sale_volm,
        'current_val' : current_period_sale_val,
        'current_volm' : current_period_sale_volm
    }
...
if __name__ == '__main__':
    ...
    # Date today
    current_date = pd.Timestamp('2018-04-15')
    ...
    sku_wise_skew_dict = calculate_sku_wise_skew(input_df, sku_master_df)
    print(sku_wise_skew_dict)
